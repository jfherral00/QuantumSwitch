...
class EntanglingConnection(Connection):
    ...
    source_fidelity_sq : float
        Probability that the source will generate a perfect pair.
    name : str, optional
...
class ControlProtocol(LocalProtocol):
    ...
    def __init__(self, network, switching_table, name=None):
        ...
        # Add SwapProtocol to configured pairs in Switch. Note: we use unique names,
...
def build_network(num_leaves, link_distance, source_fidelity_sq,switching_table,instr_duration,depol_prob):
    ...
    source_fidelity_sq : float
        Probability that the spurce will generate a perfect Bell pair.
    ...
    for route in switching_table:
        ...
        for channel_name in ['qchannel_C2A', 'qchannel_C2B']:
            qconn.subcomponents[channel_name].models['quantum_noise_model'] =\
                FibreDepolarizeModel(p_depol_init=cfg['p_depol_init'], p_depol_length=prob)
        ...
        for distance in cfg['list_distance']:
            ...
            for prob in cfg['list_probs']:
                ...
                network = build_network(num_leaves=cfg['num_leaves'],link_distance=distance, 
                                        ...
                                        switching_table=switching_table,instr_duration=instr, depol_prob=prob)
